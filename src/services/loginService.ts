...
const loginService = async (username_or_email: string, password: string): Promise<object> => {

  if (username_or_email === undefined || password === undefined) throw new statusCodeErrors("Incomplete data!", 400);

  const userDATAS: any = await findByEmailOrUsername(username_or_email);
